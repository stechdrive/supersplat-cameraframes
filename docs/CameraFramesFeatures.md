...
CAMERA FRAMES v2.14.0 は、A4相当の基準紙に複数のフレームをレイアウトし、アンカー付きの off-axis フラスタムで構図を保ちながらプレビューと書き出しを一致させるための機能群です。プレビューのズームアウトやフレーム操作、カメラポーズ編集、PSD/PNG エクスポートを一体で提供します。
...
- **カメラ二層管理**: CAMERA FRAMES 有効時はメイン構図を固定し、無効時は編集カメラで自由に確認。編集視点でメインを調整する場合はサブパネルから行う。
- **書き出しオプション**: PNG/PSD、グリッド＋アイレベル同時出力、モデルレイヤー(PSD)対応。150dpi pHYs、プレマルチ解除、フレーム別レイヤーを自動付与。

## 2. パネルとトグル
- ヘッダーに **CAMERA FRAMES ON/OFF トグル**（Main=ON、Viewport=OFF アイコン）と **メインカメラプロパティ**（サブパネル）ボタン、**コンパクト切替**。コンパクト時はヘッダーのみ表示。
- メインカメラプロパティは **CAMERA FRAMES OFF 時のみ**開ける。開いている間はメイン編集モードとしてフラスタム強調を継続。
- パネルはドラッグで移動し、ウィンドウ内にクランプ。パネル上のポインタ操作はキャンバスへ伝搬しない。
- ラベルに `| CAMERA FRAMES v2.14.0` を併記。
...
- **FOV(mm)**: メインカメラ表示時のみ、構図基準の水平 FOV を 35mm 換算で表示・編集（HFOV 10–120°）。編集視点でメインを調整する場合はサブパネルの FOV を使う。
...
## 7. カメラ／ビュー／トランスフォーム
- CAMERA FRAMES ON/OFF で **メイン表示**と**編集表示**を切り替える。編集表示中も mainPose を保持し、サブパネルでメインカメラ編集が可能。
