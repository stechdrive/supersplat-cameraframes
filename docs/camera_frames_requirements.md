...
- ベースコード: `src/camera-frames.ts` / `src/ui/camera-frames-panel.ts` / `src/camera.ts` / `src/scene.ts` / `src/render.ts`（package version 2.17.0 / HEAD 時点）。
...
- CAMERA FRAMES 個別バージョン: `cameraFramesVersion` = **v2.17.0**（`package.json` 由来、`#app-label` に `| CAMERA FRAMES v2.17.0` を追加表示）。
