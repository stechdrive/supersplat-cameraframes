...
- Base code: `src/camera-frames.ts` / `src/ui/camera-frames-panel.ts` / `src/camera.ts` / `src/scene.ts` / `src/render.ts` (package version 2.17.0 / as of HEAD).
- CAMERA FRAMES individual version: `cameraFramesVersion` = **v2.17.0** (from `package.json`, appended as `| CAMERA FRAMES v2.17.0` to `#app-label`).
