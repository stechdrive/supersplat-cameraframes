...
- Base code: `src/camera-frames.ts` / `src/ui/camera-frames-panel.ts` / `src/camera.ts` / `src/scene.ts` / `src/render.ts` (package version 2.16.6 / as of HEAD).
- CAMERA FRAMES individual version: `cameraFramesVersion` = **v2.16.2** (from `package.json`, appended as `| CAMERA FRAMES v2.16.2` to `#app-label`).
